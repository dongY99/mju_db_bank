...
def add_deposit_account():
    try:
        ...
        Customer_Resident_Registration_Number = data.get('Customer_Resident_Registration_Number')

        # 필수 필드가 없으면 오류 반환
        if not Deposit_Account_ID or not Account_Type or not Data_Of_Opening or not Customer_Resident_Registration_Number:
            return jsonify({"error": "계좌ID, 계좌종류, 개설날짜, 고객주민번호는 필수 항목입니다."}), 400
...
def add_transaction():
    try:
        ...
        # 필수 필드가 없으면 오류 반환
        if not Deposit_Account_ID or not Transaction_Amount or not Data_Of_Deposit_Withdrawal:
            return jsonify({"error": "출금계좌, 거래금액, 거래날짜는 필수 항목입니다."}), 400
        ...
        new_Balance = deposit_account.Balance + Transaction_Amount  # 입금(+) 또는 출금(-)
...
def add_card():
    try:
        ...
        # 필수 필드가 없으면 오류 반환
        if not Card_ID or not Card_Type or not Limit_Amount or not Payment_Date or not Date_Of_Application or not Customer_Resident_Registration_Number:
            return jsonify({"error": "카드ID, 카드종류, 한도금액, 결제날짜, 신청일자, 고객주민번호는 필수 항목입니다."}), 400
...
def query_accounts_by_name_sorted(name):
    query = (
        db.session.query(DepositAccount.Deposit_Account_ID, DepositAccount.Balance, DepositAccount.Data_Of_Opening)
        .join(Customer)
        .filter(Customer.Name == name)
...
def query_recent_transactions(name, account_id):
    ...
    query = (
        db.session.query(Transaction.거래번호, Transaction.Data_Of_Deposit_Withdrawal, Transaction.Transaction_Amount, Transaction.Details_Of_Transaction)
        .join(DepositAccount)
        .join(Customer)
...
def query_cards_by_name(name):
    query = (
        db.session.query(Card.Card_ID, Card.Card_Type, Card.Limit_Amount, DepositAccount.Deposit_Account_ID)
        .join(Customer)
        .outerjoin(DepositAccount, Card.Deposit_Account_ID == DepositAccount.Deposit_Account_ID)
        .filter(Customer.Name == name)
    )
...
def query_next_birthday():
    ...
    query = (
        db.session.query(
            Customer.Name, Customer.Address, Customer.Date_Of_Birth
        )
        .filter(func.day(Customer.Date_Of_Birth) >= func.day(today))
        .order_by(
            func.day(Customer.Date_Of_Birth) - func.day(today)  # 생일이 가까운 순으로 정렬
        )
        .first()
    )
...
def update_deposit_account(Deposit_Account_ID):
    try:
        ...
        account.Card_Application_Status = data.get('Card_Application_Status', account.Card_Application_Status)
        account.Data_Of_Opening = data.get('Data_Of_Opening', account.Data_Of_Opening)

        # 데이터베이스 커밋
        db.session.commit()

        return jsonify({"message": "Deposit account updated successfully", "deposit_account": {
        ...
        db.session.rollback()
        return jsonify({"error": "Error updating deposit account", "details": str(e)}), 500
...
def update_card(Card_ID):
    try:
        ...
        return jsonify({"message": "Card updated successfully", "card": {
